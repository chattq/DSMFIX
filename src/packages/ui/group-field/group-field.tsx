...
export const GroupField = ({item, formData, disableCollapsible}: GroupFieldProps) => {
  const control = useVisibilityControl({defaultVisible: true})
  return (
    <div className={'form-group'}>
      <GroupHeader caption={item.caption} control={control} disableCollapsible={disableCollapsible} />
      <Form
        ...
        {item.items?.map((subItem: any, subIndex: number) => {
          return (
            <SimpleItem {...subItem} key={subIndex}/>
          )
        })}
      </Form>
    </div>
