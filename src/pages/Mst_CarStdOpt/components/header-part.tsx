...
export const HeaderPart = ({ onAddNew }: HeaderPartProps) => {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    const handleResult = selectedItems.reduce(
      (prev: any, cur: any) => {
        return {
          ...prev,
          ListModelCode: [...prev.ListModelCode, cur.ModelCode],
          ListStdOptCode: [...prev.ListStdOptCode, cur.StdOptCode],
          ListGradeCode: [...prev.ListGradeCode, cur.GradeCode],
        };
      },
      {
        ListModelCode: [],
    ...
    const result = {
      ListModelCode: handleResult.ListModelCode.join(","),
      ListStdOptCode: handleResult.ListStdOptCode.join(","),
      ListGradeCode: handleResult.ListGradeCode.join(","),
    };

    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_CarStdOpt_ExportByListCode(result);
      })
      .otherwise(async () => {
        return await api.Mst_CarStdOpt_Export(searchParamsAtomValue);
      });
    if (resp.isSuccess) {
      toast.success(t("Download Successfully"));
      window.location.href = resp.Data;
  ...
  return (
    <PageHeaderNoSearchLayout>
      <PageHeaderNoSearchLayout.Slot name={"Before"}>
        <div className="font-bold dx-font-m">{t("Mst_CarStdOpt")}</div>
