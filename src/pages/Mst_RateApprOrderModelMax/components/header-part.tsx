...
export const HeaderPart = ({ onAddNew }: HeaderPartProps) => {
  ...
  const onDownloadTemplate = async () => {
    const resp = await api.Mst_RateApprOrderModelMax_ExportTemplate();
    if (resp.isSuccess) {
  ...
  const handleExportExcel = async (selectedOnly: boolean) => {
    const handleResult = selectedItems.reduce(
      (prev: any, cur: any) => {
        return {
          ...
          ListDealerCode: [...prev.ListDealerCode, cur.DealerCode],
        };
      },
      {
        ListModelCode: [],
    ...
    const result = {
      ...
      ListDealerCode: handleResult.ListDealerCode.join(","),
    };

    let resp = await match(selectedOnly)
      .with(true, async () => {
        return await api.Mst_RateApprOrderModelMax_ExportByListDealerCode(
          result
        );
      })
      .otherwise(async () => {
        return await api.Mst_RateApprOrderModelMax_Export(
