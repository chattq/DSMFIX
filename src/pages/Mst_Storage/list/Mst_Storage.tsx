...
import { FlagActiveEnum, Mst_Storage, SearchParam } from "@/packages/types";
...
export const Mst_StoragePage = () => {
  const { t } = useI18n("Mst_Storage");
  ...
  const onCreate = async (data: Partial<Mst_Storage>) => {
    const respone = await api.Mst_Storage_Create(data);
  ...
  const onDelete = async (key: any) => {
    const respone = await api.Mst_Storage_Delete(key?.StorageCode);
    if (respone.isSuccess) {
  ...
  const onUpdate = async (key: string[], data: Partial<Mst_Storage>) => {
    const respone = await api.Mst_Storage_Update(key, data);
    if (respone.isSuccess) {
  ...
  const handleExportFile = async (selectedOnly: boolean) => {
    const selectedKeys = selectedOnly
      ? gridRef.instance.getSelectedRowKeys()
      : [];
    const resp = await api.Mst_Storage_ExportExcel(selectedKeys);
    if (resp.isSuccess) {
  ...
  const handleEditorPreparing = (e: EditorPreparingEvent<any, any>) => {
    if (
      ...
      e.dataField === "ProvinceCode" ||
      e.dataField === "StorageType"
    ) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Mst_Storage_DeleteMultiple(
      rows.map((item: any) => {
        return {
          StorageCode: item,
        };
    ...
    <AdminContentLayout>
      <AdminContentLayout.Slot name="Header">
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name="Before">
            <div className="font-bold dx-font-m">{t("Mst_Storage")}</div>
      ...
      <AdminContentLayout.Slot name="Content">
        <BaseGridView
          ...
          inlineEditMode="row"
          keyExpr={["StorageCode", "ProvinceCode", "StorageType"]}
